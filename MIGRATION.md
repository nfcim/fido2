...
* `authenticateComplete()` returns the signature counter.
  `authenticateCompleteResult()` returns both `signCount` and `backedUp`.
...
`key.validate()` checks mathematical validity in Rust. `key.verify()` is
synchronous and throws on verification failure. `RustCrypto.verify()` returns
...
final user = PublicKeyCredentialUserEntity(
  id: [1, 2, 3], name: 'alice', displayName: 'Alice',
);
final request = server.registerBegin(user);
// Persist challenge, offeredAlgorithms and user.id with the session.
final registered = server.registerComplete(responseJson,
  expectedChallenge: request.challenge,
  offeredAlgorithms: request.offeredAlgorithms,
  userHandle: user.id,
);
```
...
```dart
final result = server.authenticateCompleteResult(assertionJson,
...
Requires Dart 3.4+, Rust 1.85+, Cargo and a platform linker. Web builds also
...
dart run tool/test.dart -p chrome -c dart2wasm test/crypto_test.dart test/webauthn_test.dart test/initialization_test.dart test/fido2_base_test.dart test/cbor_boundaries_test.dart
dart analyze
