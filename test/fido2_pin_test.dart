...
void main() {
  ...
  group('PinProtocol V1', () {
    test('encapsulate', () async {
      ...
      final peerCoseKey = EcdhEsHkdf256.fromPublicKey(
        pubBytes.sublist(0, 32),
        pubBytes.sublist(32, 64),
      );
    ...
    test('verify', () async {
      ...
      expect(
        await pinProtocol.verify(key, message, signatureFalse),
        equals(false),
      );
    });
  });
  ...
  group('PinProtocol V2', () {
    ...
    test(
      'ECDH and both HKDF keys match independent Dart implementation',
      () async {
        final ec = getP256();
        final private = ec.generatePrivateKey();
        final public = hex.decode(private.publicKey.toHex().substring(2));
        final result = await protocol.encapsulate(
          EcdhEsHkdf256.fromPublicKey(
            public.sublist(0, 32),
            public.sublist(32),
          ),
        );
    ...
    test('random IV, AES, HMAC, wire truncation and invalid lengths', () async {
      ...
      expect(
        await reference.decrypt(
          SecretBox(
            encrypted.sublist(16),
            nonce: encrypted.sublist(0, 16),
            mac: Mac.empty,
          ),
