...
void main() {
  ...
  test('SHA-256, SM3, HMAC and RFC 5869 HKDF vectors', () {
    expect(
      hex.encode(RustCrypto.sha256(utf8.encode('abc'))),
      'ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad',
    );
    expect(
      hex.encode(RustCrypto.sm3(utf8.encode('abc'))),
      '66c7f0f462eeedd9d1f2d46bdc10e4e24167c4875cf2f7a2297da02b8f4ba8e0',
    );
    expect(
      hex.encode(
        RustCrypto.hmacSha256(List.filled(20, 0x0b), utf8.encode('Hi There')),
      ),
      'b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7',
    );
    expect(
      hex.encode(
        RustCrypto.hkdfSha256(
          List.filled(22, 0x0b),
  ...
  test('NIST SP 800-38A AES-256-CBC and invalid lengths', () {
    ...
    expect(
      () => RustCrypto.aes256Cbc(key, [1], iv: iv),
  ...
  test('RFC 8032 Ed25519 test 1', () {
    final key = Ed25519.fromPublicKey(
      hex.decode(
    ...
    key.verifySync([], signature);
    expect(
      () => key.verifySync([1], signature),
      throwsA(isA<CryptoException>()),
    );
    expect(
      () => Ed25519.fromPublicKey(List.filled(32, 0)).validate(),
      throwsA(isA<CryptoException>()),
    );
  });
  test('RFC 6979 P-256 SHA-256 sample (high S accepted)', () {
    ...
    key.verifySync(utf8.encode('sample'), sig, encoding: SignatureEncoding.raw);
    expect(
      () => key.verifySync(utf8.encode('sample'), sig),
      throwsA(isA<CryptoException>()),
    );
  });
  group('SM2 GB/T 32918.2 example', () {
    ...
    test('valid raw and explicit DER', () {
      key.validate();
      key.verifySync(message, raw);
      key.verifySync(message, der, encoding: SignatureEncoding.der);
    });
    test('wrong ID, message, key, and signature', () {
      expect(
        () => key.verifySync(message, raw, sm2Id: 'wrong'),
        throwsA(isA<CryptoException>()),
      );
      expect(
        () => key.verifySync([0, ...message], raw),
        throwsA(isA<CryptoException>()),
      );
      final changed = [...raw]..[10] ^= 1;
      expect(
        () => key.verifySync(message, changed),
        throwsA(isA<CryptoException>()),
      );
      expect(
        () => SM2
    ...
    test('raw and DER encoding boundaries', () {
      ...
      for (final bad in [
        ...
        expect(
          () => key.verifySync(message, bad, encoding: SignatureEncoding.der),
          throwsA(isA<CryptoException>()),
        );
      }
      expect(
        () =>
            Sm2Configuration(algorithm: -65537, curve: -65537, id: 'x' * 8192),
        throwsArgumentError,
      );
    });
  });
  ...
  for (final entry in fixtures['rfc9964'] as List) {
    test('RFC 9964 ${entry['source']}', () {
      ...
      expect(
        () => RustCrypto.verify(algorithm, key.sublist(1), message, sig),
        throwsA(isA<CryptoException>()),
      );
      expect(
        () => RustCrypto.verify(algorithm, key, message, sig.sublist(1)),
        throwsA(isA<CryptoException>()),
      );
      expect(
        RustCrypto.verify(algorithm, key, message, sig, context: [1]),
        isFalse,
      );
      expect(
        () => RustCrypto.verify(
      ...
      expect(
        () =>
            RustCrypto.verify(algorithm, key, message, sig, mlDsaMode: 'hash'),
        throwsA(isA<CryptoException>()),
      );
