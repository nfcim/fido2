...
void main() {
  ...
  test('verified BS transitions in both directions; BE and signature enforced',
      () async {
    ...
    final client = utf8.encode(jsonEncode({
      'type': 'webauthn.get',
    ...
    for (final previous in [false, true]) {
      final stored = RegisteredCredential(
          id: [1], publicKey: key, backupEligible: true, backedUp: previous);
      for (final current in [false, true]) {
        final auth = [...rpHash, current ? 0x19 : 0x09, 0, 0, 0, 1];
        final signature = await signer
            .sign([...auth, ...RustCrypto.sha256(client)], keyPair: pair);
