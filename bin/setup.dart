...
Future<void> main(List<String> arguments) async {
  if (arguments.any((arg) => arg != '--web' && !arg.startsWith('--output='))) {
    stderr
        .writeln('Usage: dart run fido2:setup [--web] [--output=build/fido2]');
    exitCode = 64;
    return;
  }
  final library =
      await Isolate.resolvePackageUri(Uri.parse('package:fido2/fido2.dart'));
  if (library == null) throw StateError('Cannot locate fido2 package');
  final root = File.fromUri(library).parent.parent.path;
  final output = Directory(arguments
  ...
  if (web) {
    File('$root/web/fido2_crypto_loader.js')
