...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final selected = await card.transmit(Uint8List.fromList(
    ...
    if (args.contains('--sign')) {
      final selection =
          args.where((a) => a.startsWith('--algorithm=')).firstOrNull;
...
Future<void> signatureTest(UsbCcid device, int algorithm,
    {CoseConfiguration? configuration}) async {
  ...
  Future<CborMap> send(int command, Map<int, dynamic> map) async {
    final result =
        await device.transceive([command, ...cbor.encode(CborValue(map))]);
  ...
  final clientCreate = utf8.encode(jsonEncode({
  ...
  final clientGet = utf8.encode(jsonEncode(
